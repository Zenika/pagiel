...
def prepare_endpoint(endpoint, test_url, function, grouped):
    (db_url, page) = extract_url_and_page(test_url)
    return endpoint.replace("$groupUrl", db_url + "." if grouped else "").replace("$url", db_url).replace("$page", page).replace("$function", function)
...
class GraphiteClient:
    ...
    def query(self, raw_endpoint, test_url, grouped=True, function='median', tps_from="-30d", tps_to=None, max_data_points=1812, format="json"):
        endpoint = f'{self.url}/render?target={prepare_endpoint(raw_endpoint, test_url, function, grouped)}&format={format}&from={tps_from}{"&to={tps_to}" if tps_to else ""}&maxDataPoints={max_data_points}'
