...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert user url request to required file format from container")
    parser.add_argument("inputFile", help="User input file")
    parser.add_argument("--ecoIndexFile", help="file for eco-index",
                        default="/home/urlconverter/results/urls.yaml")
    parser.add_argument("--sitespeedFile", help="file for sitespeed",
                        default="/home/urlconverter/results/urls.txt")
    parser.add_argument("--yellowLabToolsFile", help="file for yellowLab tools",
                        default="/home/urlconverter/results/urls-yellowlabtools.yaml")
    parser.add_argument("--robotFolder", help="folder for robot tests",
                        default="/home/urlconverter/tests/")
    parser.add_argument("--localContainerName", help="name of the container")
    parser.add_argument("--localContainerPort",
                        help="port of the container", default=80, type=int)
...
def save_robot_files(robot_folder: str, url_list: list):
    robot_resources = [ROBOT_RESOURCE_TEMPLATE.format(
        url=url["url"], name=url["name"], final_url=url.get("final_url", url["url"])) for url in url_list]
    robot_tests = []
    for url in url_list:
        test = ROBOT_TEST_TEMPLATE.format(
            name=url["name"], cookie=ROBOT_COOKIE if url.get("cookie_btn") else "")
        if url.get("cookie_btn"):
            test = test.format(cookie_btn=url.get("cookie_btn"))
        robot_tests.append(test)

    robot_resource_file_name = os.path.join(
        robot_folder, "generated-resource.resource")
    shutil.copyfile(RESOURCE_FILES.format(
        file="resource.resource"), robot_resource_file_name)
    ...
    shutil.copyfile(RESOURCE_FILES.format(
        file="tests.robot"), robot_test_file_name)
        ...
        if args.localContainerName:
            ...
            for url in url_list:
                url["url"] = url["url"].replace(
                    "${containerName}", containerUrl, 1)
                if final_url := url.get("final_url"):
                    url["final_url"] = final_url.replace(
                        "${containerName}", containerUrl, 1)

        save_yaml_file(args.ecoIndexFile, url_list, GREENIT_INPUT_FILE_ARGS)
        save_yaml_file(args.yellowLabToolsFile, url_list,
                       YELLOWLABTOOLS_INPUT_FILE_ARGS)
